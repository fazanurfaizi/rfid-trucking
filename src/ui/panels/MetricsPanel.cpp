...
MetricsPanel::MetricsPanel() : Border(BorderStyle::Rounded) {
...
void MetricsPanel::update() {
  ...
  auto diff =
      std::chrono::duration_cast<std::chrono::seconds>(now - this->start_time_)
          .count();
  int h = diff / 3600;
  int m = (diff % 3600) / 60;
  int s = diff % 60;
  ...
  this->rx_txt_->set_text(fmt::format("{:.1f} MB/s", current_rx * 100));
  ...
  this->tx_txt_->set_text(fmt::format("{:.1f} MB/s", current_tx * 50));

  int active_conns = 12 + (rand() % 4);
  this->conn_txt_->set_text(fmt::format("{}", active_conns));
}

void MetricsPanel::switchTab(TabState tab) {
...
void MetricsPanel::buildSystemComponents() {
...
void MetricsPanel::buildNetworkComponents() {
...
std::shared_ptr<Widget> MetricsPanel::createCpuPanel() {
...
std::shared_ptr<Widget> MetricsPanel::createMemPanel() {
  auto container = std::make_shared<Vertical>();
  auto row = std::make_shared<Horizontal>();
  row->fixed_height = 1;
...
std::shared_ptr<Widget> MetricsPanel::createDiskPanel() {
  auto container = std::make_shared<Vertical>();
  auto row = std::make_shared<Horizontal>();
  row->fixed_height = 1;
  row->add(std::make_shared<Label>("DISK: "));
...
std::shared_ptr<Widget> MetricsPanel::createUptimePanel() {
  auto container = std::make_shared<Vertical>();
  auto row = std::make_shared<Horizontal>();
  row->fixed_height = 1;
  row->fixed_width = 16;
...
std::shared_ptr<Widget> MetricsPanel::createRxPanel() {
  auto container = std::make_shared<Vertical>();
  auto row = std::make_shared<Horizontal>();
  row->fixed_height = 1;
...
std::shared_ptr<Widget> MetricsPanel::createTxPanel() {
  auto container = std::make_shared<Vertical>();
  auto row = std::make_shared<Horizontal>();
  row->fixed_height = 1;
...
std::shared_ptr<Widget> MetricsPanel::createConnectionsPanel() {
  auto container = std::make_shared<Vertical>();
  auto row = std::make_shared<Horizontal>();
  row->fixed_height = 1;
