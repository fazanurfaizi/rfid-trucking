...
class CheckpointTabsPanel : public Border {
public:
  CheckpointTabsPanel(const AppConfig &config) : Border(BorderStyle::Rounded) {
...
private:
  ...
  std::shared_ptr<Widget> panel_cp_1_;
  std::shared_ptr<Widget> panel_cp_2_;
  ...
  buildCheckpointContent(const CheckpointConfig &config) {
    ...
    layout->add(std::make_shared<Label>(
        fmt::format("MQTT RFID: {}", config.mqtt_rfid)));
    ...
    layout->add(
        std::make_shared<Label>(fmt::format("Location To: {}", loc_to)));
